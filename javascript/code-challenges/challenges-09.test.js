...
const updateNumbers = (obj) => {
  let printableArray = [];
  printableArray = Object.entries(obj).map(entry => entry.join(': '));
...
const hasChildrenValues = (arr, character) => {
  let charObj=arr.filter(char=>char.name===character);
  let objValuesArr=Object.values(charObj[0]);
  objValuesArr=objValuesArr.filter(value=>typeof value ==='object');
  if (objValuesArr.length >= 1)
    return true;
  else
    return false;
};
