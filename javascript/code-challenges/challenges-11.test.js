...
function transformToLis(obj) {
  ...
  // .join(':');
  console.log('entries Arr: ',entriesArr);
  entriesArr=entriesArr.map((entry) => entry.join(': '));
  return entriesArr.map(entry=> `<li>${entry}</li>`);
}
...
let findMaleAndFemale = (data) => {
  let charArr;
  charArr=data.filter(char=>char.gender==='male'||char.gender==='female');
  charArr=charArr.map(char=> char.name).join(' and ');
  return charArr;
};
...
let findShortest = (data) => {
  let shorty= data.reduce((prev,curr)=>{
    if(prev.height>parseInt(curr.height)){
      return curr;
    }else{
      return prev;
    }
